...
class Detail extends Component {
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    if (nextProps.match.params.id !== this.props.match.params.id) {
      ...
      http.getJson("/api/wp-json/wp/v2/posts/" + id, "", "").then((res) => {
        document.title = res.data.title.rendered + "- 郝海涛的个人网站";
        ...
        if (res.data.tags.length > 0) {
          ...
          res.data.tags.map((item) => {
            return http
              .getJson("/api/wp-json/wp/v2/tags/" + item, "", "")
              .then((res) => {
  ...
  render() {
    ...
    return (
      <ArticleContent className={loading ? "detail-loading" : ""}>
        <Spin tip="Loading..." style={{ marginTop: "25%" }} spinning={loading}>
          {leftData ? (
            <div
              className="post-nav-l"
              title="上一篇"
              onClick={this.previous.bind(this, leftData)}
            >
              <span>
                <Icon type="left" />
              </span>
          ...
          {rightData ? (
            <div
              className="post-nav-r"
              title="下一篇"
              onClick={this.next.bind(this, rightData)}
            >
              <span>
                <Icon type="right" />
              </span>
            ...
            <div className="detail-article">
              ...
              <div className="content-footer">
                <div className="article-info">
                  <div>
                    文章分类：<Tag color="blue">{blog.category_name}</Tag>
                  </div>
                  <div>
                    文章标签：
                    {tagData.map((ele) => {
